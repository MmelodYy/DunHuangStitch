...
from utils.output_tf_spatial_transform_local import Stitching_Domain_STN_Mesh
from utils.tf_spatial_transform_local import transformer as mesh_transformer
...
def H2Mesh(H2, patch_size):
    ...
    h = patch_size/grid_h
    w = patch_size/grid_w
    w_list = torch.arange(0.,patch_size+1.,w)
    w_list_arr = w_list.repeat((grid_h+1))
    h_list = torch.arange(0., patch_size + 1., h)
    h_list_arr = h_list.repeat((grid_w+1),1)
    h_list_arr = h_list_arr.T
    h_list_arr = h_list_arr.contiguous().view(-1)
    one_list = torch.ones(1)
    one_list_arr = one_list.repeat((grid_w+1)*(grid_h+1))
    ori_pt = torch.stack((w_list_arr,h_list_arr,one_list_arr),dim=0).unsqueeze(0).to(device)

    ori_pt = torch.tile(ori_pt,[batch_size, 1, 1])
    tar_pt = torch.matmul(H2, ori_pt)
    # print("tar_pt:", tar_pt.shape)
    x_s = tar_pt[:,0,:].unsqueeze(1)
    y_s = tar_pt[:,1,:].unsqueeze(1)
    z_s = tar_pt[:,2,:].unsqueeze(1)
    # print("x_s:", x_s.shape)
    H2_local = torch.cat([x_s/z_s, y_s/z_s], dim=1)
    # print("H2_local:",H2_local.shape)
    H2_local = H2_local.permute(0,2,1)
    return H2_local.view(batch_size, grid_h+1, grid_w+1, 2)

#Covert global homo into mesh
def tps_H2Mesh(H_inv, rigid_mesh):
    ori_pt = rigid_mesh.reshape(rigid_mesh.size()[0], -1, 2)
    ones = torch.ones(rigid_mesh.size()[0], (grid_h+1)*(grid_w+1),1)
    ori_pt = ori_pt.to(device)
    ones = ones.to(device)
...
class MeshRegressionNet(nn.Module):
    def __init__(self,fIndim=256,
                 pretrain_img_size=[64, 64],
                 embed_dim=16,
                 depths=[3, 3, 3, 3],
        ...
        super(MeshRegressionNet, self).__init__()
        ...
        self.fc = nn.Sequential(
            ...
            # nn.GELU(),
            # nn.Dropout(0.5),
            nn.Linear(1024,(grid_w+1)*(grid_h+1)*2)
        )
    ...
    def forward(self,x):
        ...
        # print("regdown:",regdown[-1].shape)
        x = self.fc(regdown[-1])
        return x.view(-1, grid_h+1, grid_w+1, 2)
...
class HModel(nn.Module):
    def __init__(self,inplanes):
        ...
        self.feature_extractor =  FeatureExtractor(num_input_channels=inplanes,
                 ...
                 out_indices=(0, 1, 2, 3))
        print("---------------reg1----------------------")
        self.corr1 = AttentionCostVolume(16) #AttentionCostVolume(3)#
        self.reg1 = RegressionNet(pretrain_img_size=[16, 16],
                 ...
                 fIndim=1024)
        print("---------------reg2----------------------")
        self.corr2 =  AttentionCostVolume(8) #AttentionCostVolume(3)#
        self.reg2 = RegressionNet(pretrain_img_size=[32, 32],
                  ...
                  fIndim=1024)
        print("---------------reg3----------------------")
        self.corr3 =  AttentionCostVolume(4) #AttentionCostVolume(3)#
        self.reg3 = MeshRegressionNet(pretrain_img_size=[64, 64],
                 embed_dim=49,
                 depths=[2, 2, 2],
                 num_heads=[7, 7, 7],
                 window_size=[4, 4, 4],
    ...
    def DLT_transform(self, img, net1_f, patch_size=32., s=4., isNorm=True):
        ...
        return feature2_warp, H1_inv, H1_mat
    ...
    def forward_once(self, x1, x2):
        ...
        search_range = 4
        patch_size = 32.
        stride = 16.  # float(self.crop_size // patch_size)
        ...
        H1_motion = self.reg1(global_correlation)
        # print("H1_motion:",H1_motion.shape)
        H1_motion = H1_motion.unsqueeze(2)
        feature2_warp, _, _ = self.DLT_transform(f2[-2], H1_motion, patch_size, stride, isNorm=True)
        # print("feature2_warp:",feature2_warp.shape)
        ############# regressionNet 2 #############
        search_range = 4
        patch_size = 64.
        stride = 8.  # float(self.crop_size // patch_size)
        ...
        H2_motion = self.reg2(local_correlation_2)
        H2_motion = H2_motion.unsqueeze(2)
        feature3_warp, _, _ = self.DLT_transform(f2[-3], H1_motion + H2_motion, patch_size, stride, isNorm=True)
        ...
        mesh_motion = self.reg3(local_correlation_3)
        return H1_motion, H2_motion, mesh_motion

    def output_H_estimator(self, or_input1, or_input2,input1, input2,size,imgsz=512.):
        batch_size, _, img_h, img_w = or_input1.size()
        net1_f, net2_f, net3_motion = self.forward_once(input2,input1)
        shift = net1_f + net2_f
        size = size.unsqueeze(2)
        size_tmp = torch.cat([size, size, size, size], dim=1) / imgsz
        resized_shift = torch.multiply(shift, size_tmp)
        H = self.DLT_Size_solver.solve(resized_shift, size)

        rigid_mesh = get_rigid_mesh(batch_size, img_h, img_w)
        ini_mesh = tps_H2Mesh(H, rigid_mesh)
        net3_motion = torch.stack([net3_motion[..., 0] * size[0,0,0] / imgsz, net3_motion[..., 1] * size[0,1,0] / imgsz], 3)
        mesh = ini_mesh + net3_motion
        ############# TPS Transform ############
        ...
        I_ = torch.tensor([[1., 0., width_min],
                           [0., 1., height_min],
                           [0., 0., 1.]])  # .unsqueeze(0)
        mask = torch.ones_like(or_input2)
        if torch.cuda.is_available():
            I_ = I_.to(device)
            mask = mask.to(device)
        I_mat = torch.matmul(torch.matmul(N_tensor_inv, I_), M_tensor).unsqueeze(0)

        homo_output = torch_homo_transform.transformer(torch.cat((or_input1 + 1, mask), 1), I_mat,
        ...
        tps_output = torch_tps_transform.transformer(torch.cat([or_input2 + 1, mask], 1), norm_mesh, norm_rigid_mesh, (out_height.int(), out_width.int()))
        warp1 = homo_output[:, 0:3, ...].permute(0,2,3,1)
        warp2 = tps_output[:, 0:3, ...].permute(0,2,3,1)
        mask1 = homo_output[:, 3:6, ...].permute(0,2,3,1)
        mask2 = tps_output[:, 3:6, ...].permute(0,2,3,1)
        warp1 = (warp1 - 1)
        warp2 = (warp2 - 1)
        coarsealignment = torch.cat([warp1,warp2,mask1,mask2],dim=3)
        ############# Multi Homography Transform ############
        # coarsealignment = Stitching_Domain_STN_Mesh(torch.cat([or_input1, or_input2], dim=1), rigid_mesh, mesh, size, resized_shift)
        return coarsealignment

    def forward(self, inputs_aug1, inputs_aug2, input1, inputs2, patch_size=512.):
        H1_motion, H2_motion, mesh_motion = self.forward_once(inputs_aug1, inputs_aug2)

        stride = 1.
        warp2_H1, H1_inv, H1_mat = self.DLT_transform(inputs2, H1_motion, patch_size, stride, isNorm=False)
        warp2_H2, H2_inv, H2_mat = self.DLT_transform(inputs2, H1_motion + H2_motion, patch_size, stride, isNorm=False)
        ...
        ini_mesh = H2Mesh(H2_inv, patch_size)
        ## calculate the final predicted mesh
        # print(ini_mesh.shape, mesh_motion.shape)
        mesh = ini_mesh + mesh_motion
        # print(inputs2.device, one.device, mesh.device)
        warp2_H3, one_warp_H3 = mesh_transformer(inputs2, one, mesh, patch_size)
        # print("warp2_H3:",warp2_H3.shape)
        return H1_motion, H2_motion, mesh, warp2_H1, warp2_H2, warp2_H3, one_warp_H1, one_warp_H2, one_warp_H3
